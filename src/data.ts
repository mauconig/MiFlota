...
const BASE: [string, string, number, string, number, Car['estado']][] = [
...
export function generateFleetData(): { cars: Car[]; movs: Mov[] } {
  const R = makeRng(7);

  const cars: Car[] = BASE.map((b, i) => {
    ...
    return {
      ...
      km,
      serviceCadaMeses: 6,
      lastServiceDate: addD(TODAY, -gapDias),
      vtvIn: ((i * 53) % 330) - 15,
      seguroIn: ((i * 71) % 360) - 5,
    };
  });
  ...
  cars.forEach((c) => {
    ...
    for (let k = 0; k < nE; k++) {
      ...
      movs.push({ id: id++, carId: c.id, type: 'egreso', cat, amount: Math.round(amt / 1000) * 1000, date: addD(TODAY, -d), desc });
    }
  });

  movs.push({ id: id++, carId: 'c13', type: 'egreso', cat: 'Taller', amount: 2850000, date: addD(TODAY, -5), desc: 'Motor — reparación mayor' });
