    ...
    <View style={{ gap: 18 }}>
      <View style={{ gap: 8 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <Text style={{ color: '#6b665c', fontSize: 11, fontWeight: '700', letterSpacing: 0.7 }}>
            PASO {step + 1} DE {totalSteps}
          </Text>
          <Text style={{ color: '#9b9386', fontSize: 11 }}>Registro guiado</Text>
        </View>
      ...
      <View style={{ gap: 6 }}>
        <Text style={{ color: '#16150f', fontSize: 25, lineHeight: 30, fontWeight: '800', letterSpacing: -0.5 }}>{title}</Text>
        {!!hint && <Text style={{ color: '#6b665c', fontSize: 14, lineHeight: 20 }}>{hint}</Text>}
      </View>
      ...
      <View style={{ flexDirection: 'row', gap: 10, paddingTop: 4 }}>
        {showBack && (
          <Pressable onPress={onBack} style={{ minHeight: 52, flex: 1, borderRadius: 18, borderWidth: 1, borderColor: '#d9cdb8', alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ color: '#5f5a51', fontSize: 15, fontWeight: '700' }}>Atrás</Text>
          </Pressable>
        )}
        <Pressable onPress={onNext} disabled={nextDisabled} style={{ minHeight: 52, flex: 1, borderRadius: 18, backgroundColor: nextDisabled ? '#e2dbcc' : '#16150f', alignItems: 'center', justifyContent: 'center' }}>
          <Text style={{ color: nextDisabled ? '#8d8578' : '#fffdf8', fontSize: 15, fontWeight: '700' }}>{nextLabel}</Text>
        </Pressable>
      </View>
    </View>
  );
