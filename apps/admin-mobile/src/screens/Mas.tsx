...
type MoreIcon = 'flota' | 'alertas' | 'choferes' | 'reportes' | 'perfil' | 'secciones';
...
function MoreIcon({ name }: { name: MoreIcon }) {
  ...
  if (name === 'flota') return <Svg viewBox="0 0 24 24" width={23} height={23} {...common}><Path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H8.5c-.6 0-1.2.3-1.6.8L4.5 10.6c-.9.2-1.5 1-1.5 1.9v3.5c0 .6.4 1 1 1h2" /><Circle cx="7" cy="17" r="2" /><Path d="M9 17h6" /><Circle cx="17" cy="17" r="2" /></Svg>;
  ...
  if (name === 'secciones') return <Svg viewBox="0 0 24 24" width={23} height={23} {...common}><Path d="M4 6h16M4 12h16M4 18h10" /><Path d="M17 16v5M14.5 18.5h5" /></Svg>;
...
export function Mas({ v }: { v: MobileView }) {
  const m = v.mas;
  const cards = [
    { title: 'Vehículos', sub: 'Ver y administrar la flota', action: m.navFlota, tone: '#eef0e8', icon: 'flota' as const },
    ...
    { title: 'Secciones', sub: 'Organizar los vehículos por grupo', action: m.navSecciones, tone: '#f5ead9', icon: 'secciones' as const },
  ];
