...
export function DashboardDetail({ v }: { v: MobileView }) {
  ...
  const isEarnings = detail.kind === 'earnings';
  const isBreakdown = detail.kind === 'breakdown';
  const isIncomeBreakdown = detail.kind === 'incomeBreakdown';
  const isSummaryList = isEarnings || isBreakdown || isIncomeBreakdown;
      ...
      <View style={card}>
        <View style={{ flexDirection: 'row', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 4 }}>
          <View style={{ flex: 1, minWidth: 0 }}>
            ...
            {!isSummaryList && <Text style={{ color: MUTED, fontSize: 12, marginTop: 3 }}>{detail.rows.length} {detail.rows.length === 1 ? 'resultado' : 'resultados'}</Text>}
          </View>
        </View>
        {detail.rows.length === 0 ? (
          ...
          <>
            {detail.rows.map((row, index) => (
              ...
              >
                <View style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {!isSummaryList && !!row.sub && <Text numberOfLines={1} style={{ color: MUTED, fontSize: 11, marginTop: 2 }}>{row.sub}</Text>}
                  <View style={{ height: 7, borderRadius: 4, backgroundColor: '#f0ebe0', marginTop: isSummaryList ? 6 : 8 }}>
            ...
            {isSummaryList && (
              <View style={{ borderTopWidth: 1, borderTopColor: '#f0ebe0', marginTop: 4, paddingTop: 12, flexDirection: 'row', alignItems: 'baseline', justifyContent: 'space-between', gap: 12 }}>
                <Text style={{ color: INK, fontSize: 14, fontWeight: '800' }}>Total</Text>
                <Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.75} allowFontScaling={false} style={{ color: INK, fontSize: 16, fontWeight: '800', textAlign: 'right', flexShrink: 1 }}>{detail.total}</Text>
              </View>
            )}
          </>
        )}
