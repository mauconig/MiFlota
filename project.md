...
3 frontends + 1 Fastify/SQLite backend. Latest commit: `cda27b8` ("Convert admin-mobile
from Vite web app to Expo/React Native scaffold").
...
| `apps/admin-mobile` | Expo/React Native app for the owner's phone (Expo SDK 54, expo-router 6). | **No** — needs exact `react@19.1.0`, has own `node_modules`/lockfile |
| `apps/driver` | Driver app placeholder (view debt, register payments, upload comprobantes). One "Todavía no hay nada acá" screen. | Yes |
...
`npm run dev:admin-mobile` (Expo/Metro :8081), `npm run dev:driver` (:5176).
...
Expo SDK 54, react-native 0.81.5, react 19.1.0 (exact), expo-router (single route —
...
(LAN IP of the API, e.g. `http://192.168.100.34:3000`).
